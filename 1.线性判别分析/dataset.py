...
for ax, column in zip(axes.ravel(), range(X.shape[1])):
    ...
    for i, color in zip(y_c, ('blue', 'red', 'green')):
        ax.hist(X[y == i, column], color=color, label='class %s' % labels[i], bins=bins, alpha=0.5, )
    ylims = ax.get_ylim()
...
np.testing.assert_array_almost_equal(
    np.mat(np.linalg.inv(S_W) * S_B) * np.mat(eigvecs[:, 0].reshape(4, 1)),
    eigvals[0] * np.mat(eigvecs[:, 0].reshape(4, 1)),
